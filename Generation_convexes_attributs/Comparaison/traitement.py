...
with open('res_24/time_find_convex.csv', 'r') as file:
...
with open('res_24/time_convex_subset.csv', 'r') as file:
    reader = csv.reader(file)
    nb_nodes = 2
    for row in reader:
        mapped_row = list(map(float, row))
...
with open('res_24/time_total.csv', 'r') as file:
    reader = csv.reader(file)
    i = 0
    for row in reader:
        mapped_row = list(map(float, row))
...
plt.plot([*range(2, 25)], total[0], 'g', label="new find convex")
plt.plot([*range(2, 25)], total[1], 'r', label="old find convex")
